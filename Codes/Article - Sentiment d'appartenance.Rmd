...
# VD1 : sentiment d'appartenance (Québec)
...
La distribution est asymétrique vers la gauche. 
L'indice moyen du sentiment d'appartenance au Québec est 0.53.

# VD2 : sentiment d'appartenance (Québec, Canada et pays d'origine)
...
Base_Trajipaq_Finale_Ok <-
  Base_Trajipaq_Finale_Ok %>%
  rowwise() %>%
  mutate(VD2 = (sum(c_across(all_of(v_VD2)), na.rm = TRUE) - length(v_VD2)) / (3*length(v_VD2))) %>%
  mutate(VD2 = if (VD2 < 0) NA_real_ else VD2) %>%
  ungroup()
...
descr(Base_Trajipaq_Finale_Ok$VD2)

```

```{r}

ggplot(data = Base_Trajipaq_Finale_Ok, mapping = aes(x = VD2)) +
  geom_histogram(na.rm = TRUE, fill = "pink", binwidth = 0.01) +
  theme_bw()
...
# VI1 : discrimination subie au cours des 12 derniers mois
...
## VI1 : L'indice de discrimination lors des 12 derniers mois précédent la crise du covid 19
...
Base_Trajipaq_Finale_Ok <-
  Base_Trajipaq_Finale_Ok %>%
  rowwise() %>%
  mutate(VI1 = (sum(c_across(all_of(v_VI1)), na.rm = TRUE) - length(v_VI1)) / (2.25*length(v_VI1))) %>%
  mutate(VI1 = if (VI1 < 0) NA_real_ else VI1) %>%
  ungroup()
...
descr(Base_Trajipaq_Finale_Ok$VI1)

```

```{r}

ggplot(data = Base_Trajipaq_Finale_Ok, mapping = aes(x = VI1)) +
  geom_histogram(na.rm = TRUE, fill = "pink", binwidth = 0.01) +
  theme_bw()
...
ctable(x = Base_Trajipaq_Finale_Ok$Q39E_N, # Sentiment d'appartenance au Canada
       y =  Base_Trajipaq_Finale_Ok$Q39F_N # Sentiment d’appartenance au pays d'origine
       )
...
Base_Trajipaq_Finale_Ok <-
  Base_Trajipaq_Finale_Ok %>%
  mutate(VD2_F = case_when(
    VD2 < 0.25 ~ "Faible",
    VD2 >= 0.25 & VD2 < 0.5 ~ "Moyen",
    VD2 >= 0.5 & VD2 < 0.75 ~ "Fort",
    VD2 >= 0.75 & VD2 <= 1 ~ "Tres fort",
  ))

Base_Trajipaq_Finale_Ok$VD2_F <- factor(Base_Trajipaq_Finale_Ok$VD2_F, levels = c("Tres fort",
                                                                                "Fort",
                                                                                "Moyen",
                                                                                "Faible"
                                                                                ), ordered = TRUE)
...
Base_Trajipaq_Finale_Ok <-
  Base_Trajipaq_Finale_Ok %>%
  mutate(VI1_F = case_when(
    VI1 < 0.25 ~ "Faible",
    VI1 >= 0.25 & VI1 < 0.5 ~ "Moyen",
    VI1 >= 0.5 & VI1 < 0.75 ~ "Fort",
    VI1 >= 0.75 & VI1 <= 1 ~ "Tres fort",
  ))

Base_Trajipaq_Finale_Ok$VI1_F <- factor(Base_Trajipaq_Finale_Ok$VI1_F, levels = c("Tres fort",
                                                                                "Fort",
                                                                                "Moyen",
                                                                                "Faible"
                                                                                ), ordered = TRUE)
...
ctable(x = Base_Trajipaq_Finale_Ok$VI1_F, y = Base_Trajipaq_Finale_Ok$VD2_F)

```

```{r}

ggplot(data = Base_Trajipaq_Finale_Ok, mapping = aes(x = VI1_F, fill = VD2_F)) +
...
chisq.test(x = Base_Trajipaq_Finale_Ok$VI1_F, y = Base_Trajipaq_Finale_Ok$VD2_F)
...
reg_1 <- lm(Base_Trajipaq_Finale_Ok$VI1 ~ Base_Trajipaq_Finale_Ok$VD2 +
              Base_Trajipaq_Finale_Ok$Q10 +     # Sexe
              Base_Trajipaq_Finale_Ok$Q8B +     # Âge
              Base_Trajipaq_Finale_Ok$STATUT +  # Statut matrimonial
              Base_Trajipaq_Finale_Ok$Q4 +      # Lieu de naissance
              Base_Trajipaq_Finale_Ok$Q16 +     # Niveau d'éducation
              Base_Trajipaq_Finale_Ok$Q17 +     # Religion
              Base_Trajipaq_Finale_Ok$Q20 +     # Langue maternelle
              Base_Trajipaq_Finale_Ok$Q23       # Etat de santé
            )
summary(reg_1)
...
cor.test(Base_Trajipaq_Finale_Ok$VI1, Base_Trajipaq_Finale_Ok$VD2)
...
vis <- ggplot(Base_Trajipaq_Finale_Ok %>%
                filter(!is.na(VD2), !is.na(VI1)), aes(x = VI1, y = VD2)) +
  geom_jitter(color = "pink") +
  stat_smooth(method = "lm", formula = "y ~ x", color = "purple",se = FALSE) +
...
vis_sex <- ggplot(Base_Trajipaq_Finale_Ok %>%
                filter(!is.na(VD2), !is.na(VI1), !is.na(Q10)), aes(x = VI1, y = VD2, color = as.factor(Q10))) +
  geom_jitter(color = "pink") +
...
vis_age <- ggplot(Base_Trajipaq_Finale_Ok %>%
                filter(!is.na(VD2), !is.na(VI1), !is.na(Q8A_F)), aes(x = VI1, y = VD2, color = Q8A_F)) +
  geom_jitter(color = "pink") +
  stat_smooth(method = "lm", formula = "y ~ x", se = FALSE) +
...
vis_born <- ggplot(Base_Trajipaq_Finale_Ok %>%
                filter(!is.na(VD2), !is.na(VI1), !is.na(STATUT_F)), aes(x = VI1, y = VD2, color = STATUT_F)) +
  geom_jitter(color = "pink") +
  stat_smooth(method = "lm", formula = "y ~ x", se = FALSE) +
...
vis_born <- ggplot(Base_Trajipaq_Finale_Ok %>%
                filter(!is.na(VD2), !is.na(VI1), !is.na(Q4)), aes(x = VI1, y = VD2, color = as.factor(Q4))) +
  geom_jitter(color = "pink") +
  stat_smooth(method = "lm", formula = "y ~ x", se = FALSE) +
...
vis_educ <- ggplot(Base_Trajipaq_Finale_Ok %>%
                filter(!is.na(VD2), !is.na(VI1), !is.na(Q16)), aes(x = VI1, y = VD2, color = as.factor(Q16))) +
  geom_jitter(color = "pink") +
  stat_smooth(method = "lm", formula = "y ~ x", se = FALSE) +
...
vis_relig <- ggplot(Base_Trajipaq_Finale_Ok %>%
                filter(!is.na(VD2), !is.na(VI1), !is.na(Q17_F)), aes(x = VI1, y = VD2, color = Q17_F)) +
  geom_jitter(color = "pink") +
  stat_smooth(method = "lm", formula = "y ~ x", se = FALSE) +
...
vis_lang <- ggplot(Base_Trajipaq_Finale_Ok %>%
                filter(!is.na(VD2), !is.na(VI1), !is.na(Q20_F)), aes(x = VI1, y = VD2, color = Q20_F)) +
  geom_jitter(color = "pink") +
  stat_smooth(method = "lm", formula = "y ~ x", se = FALSE) +
...
vis_health <- ggplot(Base_Trajipaq_Finale_Ok %>%
                filter(!is.na(VD2), !is.na(VI1), !is.na(Q23)), aes(x = VI1, y = VD2, color = as.factor(Q23))) +
  geom_jitter(color = "pink") +
  stat_smooth(method = "lm", formula = "y ~ x", se = FALSE) +
