...
ggplot(data = diamonds) +
  stat_count(
    mapping = aes(x = cut, fill = cut), geom = "bar", position = "stack", show.legend = FALSE) +
  coord_flip() +
  facet_wrap(diamonds$color, nrow = 4, ) +
  theme_few()
```
...
```{r}
ggplot(data = mpg, mapping = aes(x = mpg$displ)) +
...
## 24 : écrivez un code avec geom_bar permettant de visualiser le goated geom_bar.

```{r}
ggplot(data = diamonds) +
  geom_col(mapping = aes(x = cut, y = depth, fill = cut))
```


## 25 : quelle est la spécificité de geom_col ?

```{r}
?geom_col
```

Geom_col permet de faire des diagrammes en barres représentant deux variables.

## 26 : quel est le point commun entre les arguments entre stat et geom ?

Je n'ai pas compris la question.

## 27 : quelles variables stat_smooth mobilise ?

Différents types de moyennes.

## 28 : quel est le problème de ces deux graphiques ?
...
Il faut assigner un groupe à la fonction after_stat, sinon le graphique est illisible.

## 29 : comment améliorer ce graphique ?
...
## 30 : quels paramètres contrôlent le jittering ?

Widht and height.

## 31 : comparez geom_jitter et geom_count
...
## 32 : quel est l'ajustement de la position par défaut pour geom_boxplot ? Démontrez-le avec mpg
...
## 33 : transformez un diagramme de barres en diagramme circulaire
...
## 35 : que nous dit le graphique suivant sur la relation entre cty et hwy ?
