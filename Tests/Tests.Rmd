...
lrm1 <- glm(formula = HLEV2G_2 ~ CFATINC, family = "binomial", data = an_test)
...
ggplot(an_test %>% filter(!is.na(HLEV2G_2)), aes(x = CFATINC, y = HLEV2G_2)) +
  geom_point() +
  ...
  labs(
    ...
    x = "Salaire net", 
    y = "Diplôme universitaire (0 = non ; 1 = oui)") +
  theme_few()
```
...
ggplot(an_test %>% filter(!is.na(HLEV2G_2), !is.na(AGEGP_2)), aes(x = as.factor(AGEGP_2), y = CFATINC, fill = as.factor(HLEV2G_2))) +
