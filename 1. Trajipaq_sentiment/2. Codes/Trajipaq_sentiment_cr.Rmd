...
trajipaq$WGI_2021 <- merge(trajipaq, wgidataset[wgidataset$year == 2021, ],
                           by.x = c("Q11"),
                           by.y = c("countryname"),
                           all.x = TRUE)
```
...
```{r}
WGI_V <- c("vae", "pve", "gee", "rqe", "rle", "cce")
...
WGI_n <- c("n_vae", "n_pve", "n_gee", "n_rqe", "n_rle", "n_cce")

trajipaq <-
  trajipaq %>% 
  rowwise() %>% 
  mutate (nbre_rep_WGI = sum(!is.na(across(starts_with("n_")))))

trajipaq <-
  trajipaq %>%
  rowwise() %>%
  mutate(sumWGI = sum(c_across(all_of(WGI_n)), na.rm = TRUE))%>%
  mutate(WGI = (sumWGI / (nbre_rep_WGI))) %>%
  mutate(WGI = if (nbre_rep_WGI == 0) NA_real_ else WGI) %>%
  ungroup()
