...
trajipaq <- trajipaq %>%
  left_join(wgi_origin_2021, by = c("Q11" = "countryname"), suffix = c("", "_origin_arrival")) %>%
  left_join(wgi_can, by = c("annee_qc" = "year"), suffix = c("", "_origin_2021")) %>%
  left_join(wgi_can_2021, select(wgi_can_2021, all_of(col_can_2021)), 
            by = character(0), suffix = c("", "_can_2021"))%>%
  mutate(vae_can_arrival = vae,
         pve_can_arrival = pve,
         gee_can_arrival = gee,
         rqe_can_arrival = rqe,
         rle_can_arrival = rle,
