...
v_CSD <- c("age", "education", "revenu", "sexe", "origine", "langue_mat", "statut_mat", "religion")

trajipaq <- trajipaq %>%
  filter(rowSums(!is.na(across(all_of(v_CSD)))) == length(v_CSD))
```
...
reg_WGI_can_arrival <- glm(formula = sentimapp_can_dico ~ wgi_origin_arrival_score, data = trajipaq, family = "binomial") # VD2 dichotomique
reg_WGI_can_arrival_ctrl <- glm(formula = sentimapp_can_dico ~ wgi_origin_arrival_score  + age + education + sexe + origine + revenu + langue_mat + religion + statut_mat , data = trajipaq, family = "binomial") #VD2 contrôlée par CSD

reg_WGI_arrival_diff <- glm(formula = sentimapp_can_dico ~ wgi_arrival_diff, data = trajipaq, family = "binomial")
reg_WGI_arrival_diff_ctrl <- glm(formula = sentimapp_can_dico ~ wgi_arrival_diff  + age + education + sexe + origine + revenu + langue_mat + religion + statut_mat , data = trajipaq, family = "binomial")
...
summary(reg_WGI_arrival_diff)
summary(reg_WGI_arrival_diff_ctrl)

# Au moment de l'enquête (like)

reg_WGI_can_2021 <- glm(formula = sentimapp_can_dico ~ wgi_origin_2021_score , data = trajipaq, family = "binomial") ## VD2 dichotomique
reg_WGI_can_2021_ctrl <- glm(formula = sentimapp_can_dico ~ wgi_origin_2021_score  + age + education + sexe + origine + revenu + langue_mat + religion + statut_mat , data = trajipaq, family = "binomial")#VD2 contrôlée par CSD

reg_WGI_2021_diff <- glm(formula = sentimapp_can_dico ~ wgi_2021_diff, data = trajipaq, family = "binomial")
reg_WGI_2021_diff_ctrl <- glm(formula = sentimapp_can_dico ~ wgi_2021_diff  + age + education + sexe + origine + revenu + langue_mat + religion + statut_mat , data = trajipaq, family = "binomial")
...
summary(reg_WGI_2021_diff)
summary(reg_WGI_2021_diff_ctrl)
```
...
stargazer(
 ...
 stargazer(
  reg_WGI_arrival_diff,
  reg_WGI_arrival_diff_ctrl,
  title = "Results", 
  align = TRUE, 
  type = "text"
)
